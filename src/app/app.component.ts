...
export class AppComponent implements OnInit {
    ...
    public saveToJSON() {
        ...
        const jsonString: any = JSON.stringify(this.graph);
        ...
        const test: any = combineJson(jsonObj, actions);
        // console.log(`test`);
        // console.log(test);
        // console.log(JSON.stringify(test));


        // let nesto: any = jsonString.concat({'Operations': JSON.stringify(test)});

        let nesto: any = $.extend(jsonObj, {"Operations": test});


        // const nesto: any = JSON.parse(test);
        console.log(`nesto`);
        console.log(JSON.stringify(nesto));
    }
        ...
        const jsonString: any = {
            cells: [
                ...
                },
                {
                    ...
                    z: 25,
                    labels: [
                        { attrs: { text: { text: "YES" } }, position: -30 }
                    ],
                    attrs: {}
                },
                {
                ...
                },
                {
                    ...
                    z: 27,
                    labels: [
                        { attrs: { text: { text: "NO" } }, position: -30 }
                    ],
                    attrs: {}
                },
                {
    ...
    public startDiagram() {
        ...
        time1 = setInterval(function () {
            ...
            if (
                test[index].attributes.type.charAt(0) === "f" ||
                test[index].attributes.type.charAt(0) === "e"
            ) {
                name = test[index].attributes.type
                    .substring(4, test[index].attributes.type.length)
                    .toLowerCase();
            } else {
                name = test[index].attributes.type
                    .substring(6, test[index].attributes.type.length)
                    .toLowerCase();
            }
        ...
        _.each(this.graph.getElements(), (node, index) => {
            removeColorElement(
                ...
                node.attributes.type
                    .substring(6, node.attributes.type.length)
                    .toLowerCase()
            );
        });
    }
...
function isBorderClicked(
    ...
    if (element_click_x <= strokeWidth || element_click_y <= strokeWidth) {
        borderClicked = true;
    }

    if (
        bbox.width - 5 <= element_click_x ||
...
function combineJson(jsonObj, actions) {
    ...
    // console.log(`jsonObj_length`);
    // console.log(jsonObj_length);
    // console.log(`actions_len`);
    // console.log(actions_len);

    for (let i: number = 0; i < jsonObj_length; i++) {
        for (let j: number = 0; j < actions_len; j++) {

            // console.log(jsonObj.attributes.cells.models[i].attributes.type);

            if (jsonObj.attributes.cells.models[i].attributes.type == 'app.RectangularModel') {
                if (jsonObj.attributes.cells.models[i].attributes.attrs[".label"].text == actions.Operations[j].OperationId) {
                    result.push(
                        actions.Operations[j]
                    );
                }
            } else if (jsonObj.attributes.cells.models[i].attributes.type == 'erd.WeakEntity') {
                if (jsonObj.attributes.cells.models[i].attributes.attrs.text.text == actions.Operations[j].OperationId) {
                    result.push(
                        actions.Operations[j]
                    );
                }
